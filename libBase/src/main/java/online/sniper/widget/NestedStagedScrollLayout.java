 ...
 * @author wangpeihe
 */
public class NestedStagedScrollLayout extends RelativeLayout implements NestedScrollingParent,
        NestedScrollingChild, ScrollingView {

    public static final String TAG = NestedStagedScrollLayout.class.getSimpleName();
    ...
    public NestedStagedScrollLayout(Context context) {
        this(context, null);
    }

    public NestedStagedScrollLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        initScrollView();
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        final int action = oldAction & MotionEvent.ACTION_MASK;
        if (action == MotionEvent.ACTION_DOWN) {
            isDisallowInterceptTouchEvent = false;
        }
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                if (isDisallowInterceptTouchEvent && mDistanceY > mDistanceX && mDistanceX > 0 && canScroll()) {
                    requestDisallowInterceptTouchEvent(false);
                }
                break;
            }
        }
        dispatchCancelEventIfNecessary(ev);
    ...
    private void dispatchCancelEventIfNecessary(MotionEvent ev) {
        int oldAction = ev.getAction();
        final int action = oldAction & MotionEvent.ACTION_MASK;
        int x = (int) ev.getX();
        int y = (int) ev.getY();
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                if (inHeaderView(x, y)) {
                    mLastTouchView = mHeaderView;
                } else if (inContentView(x, y)) {
                    mLastTouchView = mContentView;
                } else {
                    mLastTouchView = null;
                }
                break;
            case MotionEvent.ACTION_MOVE:
                if (mLastTouchView == mHeaderView) {
                    if (!inHeaderView(x, y)) {
                        dispatchCancelEvent(mHeaderView, ev);
                        if (inContentView(x, y)) {
                            mLastTouchView = mContentView;
                        } else {
                            mLastTouchView = null;
                        }
                    }
                } else if (mLastTouchView == mContentView) {
                    if (!inContentView(x, y)) {
                        dispatchCancelEvent(mContentView, ev);
                        if (inHeaderView(x, y)) {
                            mLastTouchView = mHeaderView;
                        } else {
                            mLastTouchView = null;
    ...
    private void dispatchCancelEvent(View view, MotionEvent ev) {
        int oldAction = ev.getAction();
        float x = ev.getX();
        float y = ev.getY();
        ev.setAction(MotionEvent.ACTION_CANCEL);
        ev.setLocation(view.getMeasuredWidth() / 2, view.getMeasuredHeight() / 2);
    ...
    private boolean inHeaderView(int x, int y) {
        if (mInflated) {
            final int scrollY = getScrollY();
            final int left = mHeaderView.getLeft();
            final int top = mHeaderView.getTop();
            final int right = mHeaderView.getRight();
            final int bottom = mHeaderView.getBottom();
            return !(y < top - scrollY
                    || y >= bottom - scrollY
                    || x < left
                    || x >= right);
    ...
    private boolean inContentView(int x, int y) {
        if (mInflated) {
            final int scrollY = getScrollY();
            final int left = mContentView.getLeft();
            final int top = mContentView.getTop();
            final int right = mContentView.getRight();
