 ...
 * {@link WebSocketReader.FrameCallback} will happen on this thread as a result. This is the only thread that
...
public abstract class RealWebSocket implements WebSocket, WebSocketReader.FrameCallback {
      ...
      @Override protected void execute() {
        if (writerClosed.compareAndSet(false, true)) {
          ...
          if (protocolException || runtimeException) {
            int code = protocolException ? WebSocketProtocol.CLOSE_PROTOCOL_EXCEPTION : WebSocketProtocol.CLOSE_CLIENT_GOING_AWAY;
            try {
              writer.writeClose(code, null);
            } catch (IOException inner) {
              Platform.get()
                  .log(Platform.INFO, "Unable to send close in response to listener error.", inner);
            }
          }
        }
  ...
  void quietlyCloseConnection() {
    ...
    try {
      ...
      Platform.get().log(Platform.INFO, "Unable to close web socket connection.", inner);
    }
  }
