...
abstract class ParameterHandler<T> {
     ...
     * 自定义处理Cookie
     *****************************************************************************************/
    static final class Cookie<T> extends ParameterHandler<T> {
        private final String name;
        private final Converter<T, String> valueConverter;

        Cookie(String name, Converter<T, String> valueConverter) {
            this.name = Utils.checkNotNull(name, "name == null");
            this.valueConverter = valueConverter;
        }
        ...
        void apply(RequestBuilder builder, T value) throws IOException {
            if (value == null) return; // Skip null values.
            builder.addCookie(name, valueConverter.convert(value));
        }
    }

    static final class CookieMap<T> extends ParameterHandler<Map<String, T>> {
        private final Converter<T, String> valueConverter;

        CookieMap(Converter<T, String> valueConverter) {
            this.valueConverter = valueConverter;
        }
        ...
        void apply(RequestBuilder builder, Map<String, T> value) throws IOException {
            if (value == null) {
                throw new IllegalArgumentException("Cookie map was null.");
            }

            for (Map.Entry<String, T> entry : value.entrySet()) {
                String cookieName = entry.getKey();
                if (cookieName == null) {
                    throw new IllegalArgumentException("Cookie map contained null key.");
                }
                T cookieValue = entry.getValue();
                if (cookieValue == null) {
                    throw new IllegalArgumentException(
                            "Cookie map contained null value for key '" + cookieName + "'.");
                }
                builder.addCookie(cookieName, valueConverter.convert(cookieValue));
            }
        }
    }
